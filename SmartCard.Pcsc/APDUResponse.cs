...
namespace SmartCard.Pcsc
{
	...
	public class APDUResponse
	{
		...
		public APDUResponse(byte[] baData)
		{
			...
			{
				this.m_baData = new byte[ baData.Length - SW_LENGTH ];

				for (int nI = 0; nI < baData.Length - SW_LENGTH; nI++)
					this.m_baData[nI] = baData[nI];
			}
			this.m_bSw1 = baData[baData.Length - 2];
			this.m_bSw2 = baData[baData.Length - 1];
		}
		...
		public ushort Status
		{
			get
			{
                ...
                //sw[0] = this.m_bSw1;
                //sw[1] = this.m_bSw2;
                //return BitConverter.ToUInt16(sw, 0);
				return (ushort)(((ushort)m_bSw1 << 8) + (ushort)m_bSw2);
			}
		}
        ...
        public override string ToString()
        {
            ...
            // Display SW1 SW2
            sRet = string.Format("SW={0:X04}", this.Status);
