...
namespace SmartCard.Player
{
	...
	public class APDUPlayer
	{
        ...
        /// CardNative property
        /// </summary>
        public ICard CardNative { get; set; }
        ...
        {
            this.LoadAPDUFile(apduFileName);
            this.LoadSequenceFile(seqFileName);
        ...
        /// </summary>
        /// <param name="fileName">APDU file name</param>
        public void LoadAPDUFile( Stream  st )
        {
            XmlDocument apduDoc = new XmlDocument();
        ...
        public void LoadSequenceFile(Stream st)
        {
            XmlDocument apduDoc = new XmlDocument();
        ...
        public void LoadCommandFile(Stream st)
        {
            XmlDocument apduDoc = new XmlDocument();
		...
		public string[] APDUNames
		{
			get
			{
				...
				int	nCount = this.m_xmlApduList.Count;
		...
		{
			APDUCommand apduCmd = this.APDUByName(apduName);
			if (apduCmd == null)
		...
		public APDUResponse ProcessCommand(string apduName)
		{
			...
			apduCmd = this.APDUByName(apduName);
			if(apduCmd == null)
                throw new ApduCommandException(ApduCommandException.NoSuchCommand);

			return this.ExecuteApduCommand(apduCmd);
		}
        ...
        public APDUResponse ProcessSequence(string apduSequenceName, SequenceParameter seqParam)
        {
            ...
            for (int nI = 0; nI < xmlCmdList.Count; nI++)
            {
                ...
                if (xmlCmdList.Item(nI) is XmlComment || xmlCmdList.Item(nI) is XmlCDataSection)
                {
                    continue;
                }
                apduResp = this.ProcessSeqCmd(xmlCmdList.Item(nI), l_seqParam);
            }
        ...
        private APDUCommand APDUFromXml(XmlNode xmlApdu)
        {
            ...
            this.m_bLeData = false;
            this.m_bCheckSW1 = false;
            ...
            try
            {
                sP3 = (string)xmlApdu.Attributes["P3"].Value;
            }
            catch
            {
                sP3 = "0";
            ...
            try
            {
                sLe = (string)xmlApdu.Attributes["Le"].Value;
            }
            catch
            {
                sLe = "-1";
            ...
            try
            {
                sData = apduElt.GetAttribute("Data");
            }
            catch
            {
                sData = "";
            ...
            if (sP3.IndexOf("DR") != -1)
            {
                ...
                try
                {
                    ...
                    if ((m_apduResp != null) && (m_apduResp.Data != null))
                        bLe = new byte[] { m_apduResp.Data[nIdx - 1] };
                    else
                        bLe = null;
                }
                catch
            ...
            else if ((nId = sP3.IndexOf('R')) == 0)
            {
                ...
                if (sP3[++nId] == ',')
                {
                    ...
                    if (nId2 != -1)
                    {
                        bLe = new byte[] { byte.Parse(sP3.Substring(nId + 1, nId2 - nId - 1))};
                    }
                    else
                        throw new ApduCommandException(ApduCommandException.ParamP3Format);
                }
        ...
        private APDUResponse ExecuteApduCommand(APDUCommand apduCmd)
        {
            ...
            if (m_bReplay)
            {
                ...
                else if (m_bLeData)
                {
                    ...
                    bLe = (byte)(m_apduResp.Data[m_nDataId - 1] + apduCmd.Le[0]);
                    m_bLeData = false;
                }
            ...
            for (int nI = 0; nI < xmlAttrs.Count; nI++)
            {
                ...
                switch (xmlParam.Name)
                {
                    case xmlAttrClass:
                    {
                        ...
                        {
                            apduParam.Class = byte.Parse(sVal, NumberStyles.AllowHexSpecifier);
                            //Console.Write(apduParam.Class + "\n" );
                    ...
                    //    finally
                    //    {
                    //        apduParam.Le = byte.Parse(sVal, NumberStyles.AllowHexSpecifier);
                    //    }
                    //    break;
                    //}
                    ...
                    case xmlAttrLe:
                    {
                        ...
                        {                           
                            ...
                            if (nLen != 0)
                            {
                                apduParam.Le = new byte[nLen];
                                for (int nJ = 0; nJ < nLen; nJ++)
                                {
                                    apduParam.Le[nJ] = byte.Parse(sVal.Substring(nJ * 2, 2), NumberStyles.AllowHexSpecifier);
                                }
                            }
                        }
                        break;
